...
class GXDLMSReader:
    ...
    def __init__(self, client, media, trace, invocationCounter):
        ...
        self.dataDir = os.environ.get("APP_DATA_DIR", ".")
        if not os.path.exists(self.dataDir):
            os.makedirs(self.dataDir, exist_ok=True)
            
        self.replyBuff = bytearray(8 + 1024)
        self.waitTime = 5000
        self.logFile = open(os.path.join(self.dataDir, "logFile.txt"), "w")
        self.trace = trace
        self.media = media
    ...
    def generateCertificates(self, logicalName):
        ...
        keys_dir = os.path.join(self.dataDir, "Keys")
        certs_dir = os.path.join(self.dataDir, "Certificates")
        keys384_dir = os.path.join(self.dataDir, "Keys384")
        certs384_dir = os.path.join(self.dataDir, "Certificates384")

        if not os.path.exists(keys_dir):
            os.mkdir(keys_dir)
        if not os.path.exists(certs_dir):
            os.mkdir(certs_dir)
        if not os.path.exists(keys384_dir):
            os.mkdir(keys384_dir)
        if not os.path.exists(certs384_dir):
            os.mkdir(certs384_dir)
